...
#define STREAM_FILE( stream )  ( (int)stream->descriptor.pointer )
  ...
  ft_ansi_stream_io( FT_Stream       stream,
                     ...
                     unsigned long   count )
  {
    int fd = STREAM_FILE( stream );

    k_seek( 0, fd, offset, SEEK_SET );

    int nread;
    if( k_read( &nread, fd, buffer, count ) )
        nread = -1;
                  ...
                  const char*  filepathname )
  {
    ...
    if ( k_open( &fd, filepathname, O_RDONLY, 0 ) )
    {
      FT_ERROR(( "FT_Stream_Open:" ));
    ...
    stream->descriptor.pointer = (void *)fd;
